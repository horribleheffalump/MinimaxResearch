...
namespace UKF
{
    ...
    /// <para> and for the model  x_{t+1} = Phi_1(x_t) + Phi_2(x_t) W_t, y_t = Psi_1(x_t) + Psi_2(x_t) Nu_t, W_t ~ (M_w, R_w), Nu_t ~ (M_nu, R_nu)</para>
    /// <para>Usage:</para>
    ...
    /// <para>TODO: all methods are duplicated. NOT COOL.</para>
    /// </summary>
    public class UKFilter
    {
                                     ...
                                     )
        {
            ...
            double crit = CalculateCriterionValue(Phi1, Phi2, Psi1, Psi2, Mw, Rw, Mnu, Rnu, Crit, p1, p2, T, models, xhat0, DX0Hat);
            //double crit2 = CalculateCriterionValue(Phi1, Psi1, Rw, Rnu, Crit, p1, p2, T, models, xhat0, DX0Hat);
            //Console.WriteLine(crit - crit2);
        ...
        /// </summary>
        /// <param name="Phi">State transformation: a nonlinear function which determines the dynamics: x_{t+1} = Phi(x_t) + W_t</param>
        /// <param name="Psi">Observations transformation: a nonlinear function which determines the relation between the state and the observations: y_t = Psi(x_t) + Nu_t</param>
        /// <param name="Rw">Covariance matrix of the state disturbances</param>
        /// <param name="Rnu">Convariance matrix of the observation noise</param>
        /// <param name="p1">Unscented transfrom parameters for the forecast phase</param>
        /// <param name="p2">Unscented transfrom parameters for the correction phase</param>
