...
namespace CMNFTest
{
        ...
        {
            {
                ...
                DW = dW;
                DNu = dNu;
                X0 = () => Exts.Vector(NormalEta[0].Sample(), NormalEta[0].Sample());
                X0Hat = mEta;
                DX0Hat = dEta;
    ...
    class TestSwitchingObservationsScalar : TestEnvironmentVector
    {
        public TestSwitchingObservationsScalar(double _dnu)
        {
            {
                TestName = "Модель с переключающимися каналами наблюдений (скалярная)";
                TestFileName = "SwitchingObservationsScalar";
                ...
                double[] switcher = new double[1000]; // 1000 so it would be enough for any reasonable T
                Normal.Samples(switcher, 0.0, 1.0); // sequence of normal distribution samples which controls the observation switches
                ...
                Func<int, Vector<double>, Vector<double>> phi1 = (s, x) => Exts.Vector(a * x[0] + b, 0);
                Func<int, Vector<double>, Matrix<double>> phi2 = (s, x) => Exts.Diag(c, 1.0);
                Func<int, Vector<double>, Vector<double>> psi1 = (s, x) => Exts.Vector(d[I(switcher[s])] * x[0]);
                Func<int, Vector<double>, Matrix<double>> psi2 = (s, x) => Exts.Matrix(sig[I(switcher[s])]);

                Phi1_latex = new string[] { @"a x_t + b", "0" };
                Phi2_latex = new string[][] { new string[] { @"c"}};
                Psi1_latex = new string[] { @"d^T * e(switcher_t) * x_t" };
                Psi2_latex = new string[][] { new string[] { @"\sigma^T * e(switcher_t)" } };
