...
inputfilename = "D:/results/sampled_UMFs_good_predict/SampledRegression_average_0.txt"
...
inputfilename = "D:/results/sampled_UMFs_good_predict/SampledRegression_average_0.txt"
xhat_UT, mErr_UT, DErr_UT, DErrTheor_UT = np.loadtxt(inputfilename, delimiter = ' ', usecols=(11,12,13,14), unpack=True, dtype=float)


#t, Dx, mErr_UMF, DErr_UMF, DErrTheor_UMF, mErr_MUMF, DErr_MUMF, DErrTheor_MUMF, mErr_UT, DErr_UT, DErrTheor_UT = multx(t), multy(Dx), multy(mErr_UMF), multy(DErr_UMF), multy(DErrTheor_UMF), multy(mErr_MUMF), multy(DErr_MUMF), multy(DErrTheor_MUMF), multy(mErr_UT), multy(DErr_UT), multy(DErrTheor_UT)
...
#ax.plot(t, DErr_UT, **params_UT, linestyle=ls_D, label='$D[x_t - \hat{x}_t]$, $D[x_{T} - \hat{x}_T] = ' + "{:.2f}".format(DErr_UT[-1]) + '$ СТ-фильтр')
...
ax.plot(t, DErr_UT, color='black', alpha = 0.5)
ax.plot(t, DErr_UMF, color='black')
ax.plot(t, DErr_MUMF, color='black', linestyle=(0, (1, 1)))
